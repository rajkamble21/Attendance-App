...
const Attendance = () => {
  ...
  const handleSignIn = async () => {
    ...
    const isPresentToday = userData.record.some(
      (entry) =>
        entry.date.slice(0, 10) === todayDate && entry.ispresent === true
    );

    if (isPresentToday) {
      const overrideConfirmation = window.confirm(
        "You are already signed in for today. Do you want to override it?"
      );
          ...
          <div class="text-center mb-4">
            {userData ? (
              userData.ispresent ? (
                ...
                <button class="btn btn-success" onClick={handleSignIn}>
                  Sign In
                </button>
              )
            ) : (
